import React from 'react';
import { Link } from "react-router-dom";
import {
  ShoppingCart,
  ArrowRight,
  CheckCircle2,
  Sparkles,
  CreditCard,
  Package,
  TrendingUp,
  Shield,
  Truck,
  BarChart3,
  Users,
  Globe
} from 'lucide-react';

export const EcommercePage = () => {
  const features = [
    {
      icon: ShoppingCart,
      title: "Panier Intelligent",
      description: "Système de panier avancé avec gestion des variations, promotions et cross-selling"
    },
    {
      icon: CreditCard,
      title: "Paiements Sécurisés",
      description: "Intégration multi-passerelles avec Stripe, PayPal et solutions locales"
    },
    {
      icon: Package,
      title: "Gestion Inventaire",
      description: "Suivi en temps réel des stocks avec alertes automatiques et réapprovisionnement"
    },
    {
      icon: Truck,
      title: "Livraison Intégrée",
      description: "Calcul automatique des frais de port et tracking des commandes en temps réel"
    },
    {
      icon: Users,
      title: "Espace Client",
      description: "Portail client complet avec historique, favoris et gestion des adresses"
    },
    {
      icon: BarChart3,
      title: "Analytics Avancés",
      description: "Tableaux de bord détaillés pour suivre vos ventes et performances"
    }
  ];

  const platforms = [
    { name: "Shopify Development", level: 94 },
    { name: "WooCommerce", level: 92 },
    { name: "Custom E-commerce", level: 96 },
    { name: "Magento", level: 85 },
    { name: "Payment Gateways", level: 98 },
    { name: "Inventory Management", level: 90 }
  ];

  const process = [
    {
      step: "01",
      title: "Stratégie E-commerce",
      description: "Analyse de votre marché, définition du catalogue produits, stratégie de prix et objectifs de vente à atteindre."
    },
    {
      step: "02",
      title: "Design & UX",
      description: "Création d'une expérience d'achat intuitive avec design attractif optimisé pour la conversion."
    },
    {
      step: "03",
      title: "Développement",
      description: "Construction de votre boutique avec intégrations paiement, livraison, CRM et gestion de stocks."
    },
    {
      step: "04",
      title: "Migration & Import",
      description: "Import de vos produits, migration de données existantes et configuration des paramètres."
    },
    {
      step: "05",
      title: "Lancement & Growth",
      description: "Mise en ligne, formation à l'utilisation et accompagnement pour optimiser vos ventes."
    }
  ];

  const packages = [
    {
      name: "Starter Shop",
      price: "",
      features: [
        "Boutique jusqu'à 50 produits",
        "Design e-commerce responsive",
        "Panier & checkout optimisé",
        "1 passerelle de paiement",
        "Gestion basique des stocks",
        "Espace client",
        "Formation 2h",
        "3 mois de support"
      ],
      popular: false
    },
    {
      name: "Growth Shop",
      price: "",
      features: [
        "Boutique jusqu'à 500 produits",
        "Design premium personnalisé",
        "Variations produits illimitées",
        "Multi-passerelles paiement",
        "Gestion stocks avancée",
        "Promotions & coupons",
        "Email marketing intégré",
        "SEO e-commerce",
        "Formation complète",
        "6 mois de support"
      ],
      popular: true
    },
    {
      name: "Enterprise Shop",
      price: "Sur devis",
      features: [
        "Produits illimités",
        "Multi-langues & devises",
        "B2B & B2C combinés",
        "ERP/CRM intégration",
        "Marketplace features",
        "IA & recommandations",
        "Analytics avancés",
        "API personnalisées",
        "Équipe dédiée",
        "Support prioritaire 24/7"
      ],
      popular: false
    }
  ];

  const stats = [
    {
      icon: ShoppingCart,
      value: "150+",
      label: "Boutiques créées",
      color: "from-blue-500 to-indigo-600"
    },
    {
      icon: TrendingUp,
      value: "+280%",
      label: "Augmentation ventes moyenne",
      color: "from-indigo-500 to-purple-600"
    },
    {
      icon: Globe,
      value: "50M€",
      label: "Volume transactions",
      color: "from-blue-500 to-purple-600"
    },
    {
      icon: Users,
      value: "98%",
      label: "Taux satisfaction client",
      color: "from-indigo-500 to-purple-600"
    }
  ];

  const ecommerceFeatures = [
    {
      title: "Gestion Produits",
      items: [
        "Catalogue illimité",
        "Variations & options",
        "Images & galeries",
        "Descriptions SEO",
        "Import/Export CSV",
        "Catégorisation avancée"
      ]
    },
    {
      title: "Commandes & Clients",
      items: [
        "Suivi des commandes",
        "Gestion des retours",
        "Comptes clients",
        "Historique d'achats",
        "Notes & communications",
        "Programmes fidélité"
      ]
    },
    {
      title: "Marketing & Ventes",
      items: [
        "Codes promo",
        "Ventes flash",
        "Bundles produits",
        "Upsell & cross-sell",
        "Avis clients",
        "Email automation"
      ]
    },
    {
      title: "Analytics & Rapports",
      items: [
        "Tableau de bord temps réel",
        "Rapports de ventes",
        "Analyse des produits",
        "Comportement clients",
        "Taux de conversion",
        "ROI marketing"
      ]
    }
  ];

  return (
    <div className="bg-white">
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 pt-32 pb-20 relative overflow-hidden">
        <div className="absolute top-20 right-10 w-96 h-96 bg-blue-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse"></div>
        <div className="absolute bottom-20 left-10 w-96 h-96 bg-indigo-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse"></div>

        <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 rounded-full border border-blue-200 mb-8">
              <ShoppingCart className="w-4 h-4 text-blue-600" />
              <span className="text-blue-700 font-semibold text-sm tracking-wide">E-COMMERCE SOLUTIONS</span>
            </div>

            <h1 className="text-5xl lg:text-7xl font-bold text-gray-900 leading-tight mb-6">
              Boutique en Ligne
              <span className="block text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600">
                Qui Convertit
              </span>
            </h1>

            <p className="text-gray-600 text-xl leading-relaxed mb-10">
              Créez une boutique en ligne performante qui transforme vos visiteurs en clients fidèles.
              Paiements sécurisés, gestion simplifiée et croissance garantie.
            </p>

            <div className="flex flex-wrap gap-4 justify-center">
              <Link to="/contact" >
                <button className="group px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold rounded-xl shadow-xl shadow-blue-500/30 transition-all duration-300 flex items-center gap-2">
                  Créer ma boutique
                  <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </button>
              </Link>
              {/* <button className="px-8 py-4 bg-white/80 backdrop-blur-sm hover:bg-white text-gray-800 font-semibold rounded-xl shadow-lg border border-gray-200 transition-all duration-300 hover:border-blue-300">
                Voir nos boutiques
              </button> */}
            </div>
            {/* Stats Grid */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-16">
              {stats.map((stat, index) => (
                <div key={index} className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-blue-100">
                  <div className={`w-12 h-12 bg-gradient-to-r ${stat.color} rounded-xl flex items-center justify-center mx-auto mb-4`}>
                    <stat.icon className="w-6 h-6 text-white" />
                  </div>
                  <div className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 mb-1">
                    {stat.value}
                  </div>
                  <div className="text-sm text-gray-600">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* Features Section */}
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-16">
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
              Fonctionnalités E-commerce
            </h2>
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              Tous les outils nécessaires pour gérer et développer votre boutique en ligne
            </p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <div
                key={index}
                className="group p-8 bg-gradient-to-br from-slate-50 to-blue-50 rounded-2xl border border-blue-100 hover:border-blue-300 transition-all duration-300 hover:shadow-xl hover:-translate-y-1"
              >
                <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                  <feature.icon className="w-7 h-7 text-white" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-3">{feature.title}</h3>
                <p className="text-gray-600 leading-relaxed">{feature.description}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Features Détaillées */}
      <section className="py-20 bg-gradient-to-br from-slate-50 to-blue-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-16">
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
              Fonctionnalités Complètes
            </h2>
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              Une solution e-commerce tout-en-un pour votre réussite
            </p>
          </div>
          <div className="grid md:grid-cols-2 gap-8">
            {ecommerceFeatures.map((feature, index) => (
              <div
                key={index}
                className="bg-white rounded-2xl p-8 border border-blue-100 hover:border-blue-300 hover:shadow-xl transition-all duration-300"
              >
                <h3 className="text-2xl font-bold text-gray-900 mb-6">{feature.title}</h3>
                <ul className="space-y-3">
                  {feature.items.map((item, idx) => (
                    <li key={idx} className="flex items-center gap-3">
                      <CheckCircle2 className="w-5 h-5 text-blue-600 flex-shrink-0" />
                      <span className="text-gray-700">{item}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Platforms Section */}
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-16">
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
              Plateformes E-commerce
            </h2>
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              Expertise sur toutes les principales solutions e-commerce
            </p>
          </div>
          <div className="max-w-4xl mx-auto space-y-6">
            {platforms.map((platform, index) => (
              <div key={index} className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 shadow-lg border border-blue-100">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-lg font-semibold text-gray-900">{platform.name}</span>
                  <span className="text-blue-600 font-bold">{platform.level}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full transition-all duration-1000"
                    style={{ width: `${platform.level}%` }}
                  ></div>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Process Section */}
      <section className="py-20 bg-gradient-to-br from-slate-50 to-blue-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-16">
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
              Processus de Création
            </h2>
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              De l'idée au lancement de votre boutique en ligne
            </p>
          </div>
          <div className="space-y-8">
            {process.map((item, index) => (
              <div
                key={index}
                className="flex gap-8 items-start group"
              >
                <div className="flex-shrink-0">
                  <div className="w-20 h-20 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center text-white font-bold text-2xl group-hover:scale-110 transition-transform duration-300 shadow-lg">
                    {item.step}
                  </div>
                </div>
                <div className="flex-grow bg-white rounded-2xl p-8 border border-blue-100 group-hover:border-blue-300 transition-all duration-300 group-hover:shadow-xl">
                  <h3 className="text-2xl font-bold text-gray-900 mb-3">{item.title}</h3>
                  <p className="text-gray-600 text-lg leading-relaxed">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Pricing Section */}
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-16">
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
              Packages E-commerce
            </h2>
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              Choisissez la solution adaptée à votre activité
            </p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {packages.map((pkg, index) => (
              <div
                key={index}
                className={`relative bg-gradient-to-br from-slate-50 to-blue-50 rounded-3xl p-8 ${
                  pkg.popular
                    ? 'border-2 border-blue-500 shadow-2xl scale-105'
                    : 'border border-blue-200 shadow-lg'
                } transition-all duration-300 hover:-translate-y-2`}
              >
                {pkg.popular && (
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                    <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-2 rounded-full text-sm font-semibold flex items-center gap-2">
                      <Sparkles className="w-4 h-4" />
                      Plus populaire
                    </div>
                  </div>
                )}

                <div className="text-center mb-8">
                  <h3 className="text-2xl font-bold text-gray-900 mb-4">{pkg.name}</h3>
                  <div className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 mb-2">
                    {pkg.price}
                  </div>
                </div>
                <ul className="space-y-4 mb-8">
                  {pkg.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <CheckCircle2 className="w-5 h-5 text-blue-600 flex-shrink-0 mt-1" />
                      <span className="text-gray-700">{feature}</span>
                    </li>
                  ))}
                </ul>
                <button 
                onClick={() => (window.location.href = "/contact")}

                className={`w-full py-4 rounded-xl font-semibold transition-all duration-300 ${
                  pkg.popular
                    ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-700 hover:to-indigo-700 shadow-lg'
                    : 'bg-white text-gray-900 hover:bg-gray-50 border border-blue-200'
                }`}>
                  Lancer ma boutique
                </button>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Payment & Security */}
      <section className="py-20 bg-gradient-to-br from-slate-50 to-blue-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="bg-white rounded-3xl p-12 border border-blue-100 shadow-xl">
            <div className="text-center mb-12">
              <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
                Paiement & Sécurité
              </h2>
              <p className="text-gray-600 text-lg">
                Transactions sécurisées et conformité garantie
              </p>
            </div>
            <div className="grid md:grid-cols-3 gap-8">
              <div className="text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center mx-auto mb-6">
                  <CreditCard className="w-8 h-8 text-white" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-3">Multi-Passerelles</h3>
                <p className="text-gray-600">Stripe, PayPal, carte bancaire et solutions locales</p>
              </div>
              <div className="text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-6">
                  <Shield className="w-8 h-8 text-white" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-3">SSL & PCI DSS</h3>
                <p className="text-gray-600">Chiffrement SSL et conformité aux normes de sécurité</p>
              </div>
              <div className="text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-6">
                  <CheckCircle2 className="w-8 h-8 text-white" />
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-3">Protection Fraude</h3>
                <p className="text-gray-600">Système anti-fraude et vérification des transactions</p>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-20 bg-gradient-to-r from-blue-600 to-indigo-600 relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20"></div>

        <div className="max-w-4xl mx-auto px-6 lg:px-12 text-center relative z-10">
          <ShoppingCart className="w-16 h-16 text-white mx-auto mb-6" />
          <h2 className="text-4xl lg:text-5xl font-bold text-white mb-6">
            Lancez Votre Boutique Aujourd'hui
          </h2>
          <p className="text-blue-100 text-xl mb-10">
            Commencez à vendre en ligne et développez votre business e-commerce
          </p>

          <div className="flex flex-wrap gap-4 justify-center">
            <Link to="/contact" >
              <button className="group px-8 py-4 bg-white text-blue-600 font-semibold rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 flex items-center gap-2 hover:scale-105">
                Demander un devis
                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </button>
            </Link>
              {/* <button className="px-8 py-4 bg-transparent border-2 border-white text-white font-semibold rounded-xl hover:bg-white hover:text-blue-600 transition-all duration-300">
                Voir des démos
              </button> */}
          </div>
        </div>
      </section>
    </div>
  );
};
